...
import type { CanvasStore } from "../store/shared/canvasTypes";
...
export function useEditorInteraction(
  ...
  const finishGroupDraw = useCallback(() => {
    ...
    if (box) {
      if (box.w > 10 && box.h > 10) {
        ...
        const cs = canvasStore?.getState() as any;
        const id = cs?.addGroupRect?.({ x: box.x, y: box.y, w: box.w, h: box.h });
        if (id) cs?.selectGroupRect?.(id, false);
      }
  ...
  const onPointerDown = useCallback((e: PIXI.FederatedPointerEvent) => {
    ...
    if (button === 0) {
      if (cs?.mode === "draw_track") {
        if (e.target && e.target.label === "background") {
          const pos = e.currentTarget.toLocal(e.global);
          let trackId = cs?.activeTrackId ?? null;
          if (!trackId) {
            trackId = cs?.addTrack?.({ nodes: [], bpm: 120, loop: false });
            cs?.setActiveTrackId?.(trackId);
          }
          const nodeId = cs?.addTrackNode?.(trackId, { x: pos.x, y: pos.y });
          cs?.setActiveNodeDrag?.({ trackId, nodeId, isNewNode: true });
      ...
      if (e.target && e.target.label === "background") {
        ...
        if (timeDiff > 50 && timeDiff < 350 && lastClickPosRef.current) {
          ...
          if (Math.hypot(dx, dy) < 20) {
            handleDoubleClickSpawn(cs, posLocal);
            lastClickTimeRef.current = 0;
            return;
          }
        }
  ...
  }, [camera, startPan, startSelection, startTrail, checkTrailIntersection, intersectedBlocksRef, canvasStore]);

  const onPointerMove = useCallback((e: PIXI.FederatedPointerEvent) => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const cs = canvasStore?.getState() as any;
    if (cs?.mode === "play") return;
    if (updatePan(e.global.x, e.global.y, cs?.updateCamera)) {
      return;
    }
    ...
    if (isSelectingRef.current) {
      ...
      if (!box) return;
      applyMarqueeSelection(box, canvasStore);
      return;
    }
    if (e.buttons === 2) {
      const pos = e.currentTarget.toLocal(e.global);
  ...
  }, [updatePan, updateSelection, updateTrail, isSelectingRef, checkTrailIntersection, canvasStore]);
  ...
  }, [endPan, endSelection, finishGroupDraw, canvasStore]);

  return { onPointerDown, onPointerMove, onPointerUp, groupDrawBox };
}

// --- internal helper: double-click spawns block / drum / groupRect / track ---
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function handleDoubleClickSpawn(state: any, posLocal: Point) {
  ...
  if (state?.mode === "drum") spawnType = "drum";
    ...
    const b = state?.blocks?.find((b: { id: string }) => b.id === state?.lastSelectedId);
  ...
  if (spawnType === "groupRect") {
    const g = state?.groupRects?.find((g: { id: string }) => g.id === state?.lastSelectedId);
    const w = g?.w || 200;
    const h = g?.h || 200;
    let nameToCopy = g?.name;
    if (nameToCopy && nameToCopy.startsWith("Group ")) nameToCopy = undefined;
    ...
    const t = state?.tracks?.find((t: { id: string }) => t.id === state?.lastSelectedId);
    if (t && t.nodes.length > 0) {
      const firstNode = t.nodes[0];
      const dx = posLocal.x - firstNode.x;
      const dy = posLocal.y - firstNode.y;
      const newNodes = t.nodes.map((n: { x: number; y: number }) => ({
        ...n, x: n.x + dx, y: n.y + dy,
...
// --- internal helper: apply marquee selection result to the canvas store ---
function applyMarqueeSelection(
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const cs = canvasStore?.getState() as any;
  const blocksList: { id: string; x: number; y: number; groupId?: string }[] = cs?.blocks ?? [];
  const tracks: { id: string; nodes: { x: number; y: number }[]; groupId?: string }[] = cs?.tracks ?? [];
  const groupRects: { id: string; x: number; y: number; w: number; h: number; groupId?: string }[] = cs?.groupRects ?? [];
