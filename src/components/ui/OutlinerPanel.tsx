...
import {
  ...
  useActiveCanvasSelectedBlockIds,
  useActiveCanvasTracks,
  useActiveCanvasSelectedTrackIds,
  useActiveCanvasCamera,
  getBlocksForContext,
  getGroupRectsForContext,
  getTracksForContext,
  getCameraForContext,
  getLastSelectedForContext,
  setSelectionBatchInContext,
  setGroupSelectionBatchInContext,
  selectBlockInContext,
  selectGroupRectInContext,
  selectTrackInContext,
...
const animateCameraTo = (
  targetX: number,
  targetY: number,
  context: CanvasContextType = "playground",
  duration = 300,
) => {
  ...
  const tick = (now: number) => {
    ...
    if (context === "game") useGameStore.getState().updateCamera(next);
      ...
      useLevelEditorStore.getState().updateCamera(next);
...
export const OutlinerPanel: React.FC = () => {
  ...
  useEffect(() => {
    const handleFind = (e: CustomEvent<string>) => {
      ...
      if (id.startsWith("groupRect:")) {
        ...
        if (tr && (tr as { enabled?: boolean }).enabled === false)
          isDisabled = true;
      } else {
        needsNotes = true;
          ...
          for (const gr of ctxGroupRects) {
            ...
            if (
              ...
              window.dispatchEvent(
                new CustomEvent("expand-group", { detail: gr.id }),
              );
              break;
            }
          }
      ...
      setTimeout(() => {
        let elId;
        if (id.startsWith("groupRect:"))
          elId = `outliner-item-groupRect-${id.split(":")[1]}`;
        else if (id.startsWith("track:"))
          elId = `outliner-item-track-${id.split(":")[1]}`;
    ...
    return () =>
      window.removeEventListener(
        "find-in-outliner",
        handleFind as EventListener,
      );
  }, [canvasContext]);

  const handleShiftClick = (
    clickedId: string,
    clickedType: "block" | "groupRect" | "track",
  ) => {
    const { lastSelectedId, lastSelectedType } =
      getLastSelectedForContext(canvasContext);
    const state = {
    ...
    if (!lastSelectedId || !lastSelectedType) {
      if (clickedType === "block")
        selectBlockInContext(canvasContext, clickedId, false);
      else if (clickedType === "groupRect")
        selectGroupRectInContext(canvasContext, clickedId, false);
      else if (clickedType === "track")
        selectTrackInContext(canvasContext, clickedId, false);
      return;
    }
    ...
    const getElId = (id: string, type: string) =>
      type === "groupRect"
        ? `outliner-item-groupRect-${id}`
        : type === "track"
          ? `outliner-item-track-${id}`
          : `outliner-item-${id}`;
    ...
    if (clickedIdx === -1 || lastIdx === -1) {
      if (clickedType === "block")
        selectBlockInContext(canvasContext, clickedId, true);
      else if (clickedType === "groupRect")
        selectGroupRectInContext(canvasContext, clickedId, true);
      else if (clickedType === "track")
        selectTrackInContext(canvasContext, clickedId, true);
      return;
    }
    ...
    rangeEls.forEach((el) => {
      const elId = el.id;
      if (elId.startsWith("outliner-item-groupRect-"))
        newSelectedGroups.add(elId.replace("outliner-item-groupRect-", ""));
      else if (elId.startsWith("outliner-item-track-"))
        newSelectedTracks.add(elId.replace("outliner-item-track-", ""));
      else if (elId.startsWith("outliner-item-"))
        newSelectedBlocks.add(elId.replace("outliner-item-", ""));
    });
  ...
  const filteredBlocks = getBlocksForContext(canvasContext).filter(
    (b) =>
  ...
  const filteredGroupRects = groupRects.filter(
    (g) =>
  ...
  const groupRectChildren = new Map<
    string,
    { blocks: typeof filteredBlocks; tracks: typeof filteredTracks }
  >();
  ...
  filteredTracks.forEach((track) => {
    ...
    for (const gr of groupRects) {
      const insideCount = track.nodes.filter(
        (n) =>
          n.x >= gr.x &&
          n.x <= gr.x + gr.w &&
          n.y >= gr.y &&
  ...
  const ungroupedBlocks = filteredBlocks.filter(
    (b) => !noteToGroupRect.has(b.id),
  );
  const ungroupedTracks = filteredTracks.filter(
    (t) => !trackToGroupRect.has(t.id),
  );
  ...
  return (
    ...
    >
      ...
      <div className="outliner-content">
        {filters.groups &&
          filteredGroupRects.map((gr, grIdx) => {
            const children = groupRectChildren.get(gr.id) || {
              blocks: [],
              tracks: [],
            };
...
const GroupRectItem = ({
  ...
  useEffect(() => {
    const handleKeyDown = (key: string, e: KeyboardEvent) => {
      if (key === "F2" && isSelected && !isEditing) {
        ...
        const totalSelectedCount =
          ctxState.selectedBlockIds.length +
          ctxState.selectedGroupRectIds.length +
          ctxState.selectedTrackIds.length;
        void lastSelectedId;
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("expand-group", handleExpand as EventListener);
  }, [groupRect.id]);
  ...
  return (
    <div className="outliner-group">
      <div
        ...
        style={{
          opacity: groupRect.enabled === false && !isSelected ? 0.4 : 1,
        }}
        ...
        onDoubleClick={(e) => {
          ...
          openContextMenuInContext(canvasContext, {
            x: e.clientX,
            y: e.clientY,
            blockId: `groupRect:${groupRect.id}`,
          });
        }}
        onContextMenu={(e) => {
          e.preventDefault();
      ...
      >
        ...
        {isEditing ? (
          <input
            ...
            onBlur={() => {
              updateGroupRectInContext(canvasContext, groupRect.id, {
                name: editName,
              });
              setIsEditing(false);
            }}
          ...
          <span
            className="font-semibold flex-1 truncate select-none"
            style={{ fontSize: "14px" }}
          >
        ...
        <button
          ...
          onClick={(e) => {
            ...
            if (isAllSelected) {
              setGroupSelectionBatchInContext(canvasContext, {
                selectedGroupRectIds: selectedGroupRectIds.filter(
                  (id) => id !== groupRect.id,
                ),
                selectedBlockIds: selectedBlockIds.filter(
                  (id) => !blockIdsToSelect.includes(id),
                ),
                selectedTrackIds: selectedTrackIds.filter(
                  (id) => !trackIdsToSelect.includes(id),
                ),
              });
            } else {
              setGroupSelectionBatchInContext(canvasContext, {
                selectedGroupRectIds: [
                  ...new Set([...selectedGroupRectIds, groupRect.id]),
                ],
                selectedBlockIds: [
                  ...new Set([...selectedBlockIds, ...blockIdsToSelect]),
                ],
                selectedTrackIds: [
                  ...new Set([...selectedTrackIds, ...trackIdsToSelect]),
                ],
          ...
          title={
            isAllSelected
              ? "Deselect Group and Children"
              : "Select Group and Children"
          }
          style={{
            width: "20px",
            height: "20px",
        ...
        <div
          ...
          title={
            groupRect.keyBinding
              ? `Key bound: ${groupRect.keyBinding}`
              : "No key bound"
          }
        >
          <Keyboard size={12} />
          <span
            ...
            style={{ fontSize: "12px", minWidth: "12px", textAlign: "center" }}
          >
            {groupRect.keyBinding || "-"}
          </span>
        </div>
...
const BlockItem = ({
  ...
  return (
    <div
      ...
      style={{
        opacity:
          (block as { enabled?: boolean }).enabled === false && !selected
            ? 0.4
            : 1,
      }}
    ...
    >
      ...
      <div
        className="outliner-key-badge"
        title={
          block.keyBinding ? `Key bound: ${block.keyBinding}` : "No key bound"
        }
      >
        <Keyboard size={12} />
        <span
...
const TrackItem = ({
  ...
  const playbackStatus = useStore(
    (state) => state.trackPlaybackStatus[track.id],
  );
  const isTrackPlaying =
    playbackStatus === "playing" ||
    (isPlaying && (track as { enabled?: boolean }).enabled !== false);
  const isTrackPaused = playbackStatus === "paused" && !isPlaying;

  useEffect(() => {
    const handleKeyDown = (key: string, e: KeyboardEvent) => {
      if (key === "F2" && selected && !isEditing) {
        const ctxState =
          canvasContext === "editor"
            ? useLevelEditorStore.getState()
            : canvasContext === "game"
  ...
  return (
    <div
      ...
      style={{
        opacity:
          (track as { enabled?: boolean }).enabled === false && !selected
            ? 0.4
            : 1,
      }}
      onPointerDown={() => (wasSelectedRef.current = selected)}
    ...
    >
      ...
      {isEditing ? (
        <input
          ...
          onChange={(e) => setEditName(e.target.value)}
          onBlur={() => {
            updateTrackInContext(canvasContext, track.id, { name: editName });
            setIsEditing(false);
          }}
          onKeyDown={(e) => {
            if (e.key === "Enter") {
        ...
        >
          ...
          {canvasContext === "playground" && isTrackPlaying && (
            <Play size={12} fill="currentColor" style={{ color: "#22c55e" }} />
          )}
          {canvasContext === "playground" && isTrackPaused && (
            <Pause size={12} fill="currentColor" style={{ color: "#eab308" }} />
          )}
        </div>
      )}
