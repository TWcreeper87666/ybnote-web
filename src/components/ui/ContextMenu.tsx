...
export const ContextMenu: React.FC = () => {
  ...
  const [measuredPos, setMeasuredPos] = React.useState<{
    x: number;
    y: number;
  } | null>(null);
  ...
  React.useEffect(() => {
    ...
    window.addEventListener("contextmenu", handleGlobalInteraction, {
      capture: true,
    });
    ...
    return () => {
      ...
      window.removeEventListener("contextmenu", handleGlobalInteraction, {
        capture: true,
      });
  ...
  const actualId = isGroupRect
    ? contextMenu.blockId.split(":")[1]
    : isTrack
      ? contextMenu.blockId.split(":")[1]
      : contextMenu.blockId;
      ...
      >
            ...
            <button
              ...
              onClick={(e) => {
                ...
                window.dispatchEvent(
                  new CustomEvent("find-in-outliner", {
        ...
        >
          ...
          <input
            ...
            value={track.bpm || 120}
            onChange={(e) => {
              getContextSt().updateTrack(track.id, {
        ...
        <div
          ...
          onClick={() =>
            getContextSt().updateTrack(track.id, {
              enabled: track.enabled === false ? true : false,
            })
          }
        >
          <span style={{ fontSize: "12px", opacity: 0.8, userSelect: "none" }}>
            Enable Track
          </span>
          <div
            className={`switch-track ${track.enabled !== false ? "active" : ""}`}
            style={{ zoom: 0.8 }}
          >
      ...
      >
        ...
        >
          <label style={{ fontSize: "12px", opacity: 0.8 }}>
            Volume: {Math.round((rect.volume ?? 1) * 100)}%
          </label>
          <input
            type="range"
            min="0"
        ...
        >
          <label style={{ fontSize: "12px", opacity: 0.8 }}>
            Pitch Offset: {rect.pitchOffset ?? 0} semitones
          </label>
          <div style={{ display: "flex", gap: "4px", alignItems: "center" }}>
            <button
              ...
              onClick={(e) => {
                e.stopPropagation();
                const cur = rect.pitchOffset ?? 0;
                if (cur > -12) getContextSt().updateGroupRect(rect.id, { pitchOffset: cur - 1 });
              }}
            >
              -1
            </button>
          ...
          <input
            ...
            onKeyDown={(e) => {
              ...
              if (e.key === "Backspace" || e.key === "Delete") {
                ...
                getContextSt().updateGroupRect(rect.id, {
        ...
        <div
          ...
          onClick={() =>
            getContextSt().updateGroupRect(rect.id, {
              enabled: rect.enabled === false ? true : false,
            })
          }
        >
          <span style={{ fontSize: "12px", opacity: 0.8, userSelect: "none" }}>
          ...
          <div
            className={`switch-track ${rect.enabled !== false ? "active" : ""}`}
            style={{ zoom: 0.8 }}
          >
            <div className="switch-thumb" />
          </div>
  ...
  const targetBlocks =
    selectedBlockIds.length > 0 && selectedBlockIds.includes(block.id)
      ? contextBlocks.filter((b) => selectedBlockIds.includes(b.id))
      : [block];

  const tunePitch = (semitones: number) => {
    getContextSt().mutateBlocks([block.id], (b) => ({
      pitch: shiftPitch(b.pitch, semitones),
      playedAt: Date.now(),
  ...
  return (
    ...
    >
        ...
        <div style={{ display: "flex", gap: "4px", marginLeft: "16px" }}>
          <button
            ...
            onClick={(e) => {
              ...
              closeContextMenu();
              window.dispatchEvent(
                new CustomEvent("find-in-outliner", { detail: block.id }),
              );
            }}
            title="Find in Outliner"
          >
        ...
        >
          ...
          <select
            ...
            onChange={(e) => {
              ...
              getContextSt().mutateBlocks([block.id], () => ({
                instrument: inst,
                playedAt: Date.now(),
                playedVolumeMultiplier: 1,
              }));
      ...
      {useGameStore.getState().gamePhase !== "arrange" && (
        ...
        >
          <label style={{ fontSize: "12px", opacity: 0.8 }}>
            {block.instrument === "percussion" ? "Drum Type" : "Pitch"}
          </label>
          {block.instrument === "percussion" ? (
            <select
              value={block.pitch}
      ...
      >
        <label style={{ fontSize: "12px", opacity: 0.8 }}>
          Volume: {Math.round((block.volume ?? 1) * 100)}%
        </label>
        <input
          type="range"
          min="0"
