...
const ResizeHandler = ({ width, height }: { width: number; height: number }) => {
...
export const PocketCanvas: React.FC<PocketCanvasProps> = ({ containerWidth, containerHeight, showOnlyMissing }) => {
  ...
  const setArrangedPocketBlocks = useStore(state => state.setArrangedPocketBlocks);
  ...
  const handlePointerDown = (e: PIXI.FederatedPointerEvent) => {
    ...
    const button = (e as unknown as { data?: { button?: number } }).data?.button ?? (e.nativeEvent as PointerEvent)?.button ?? e.button;
    ...
    } else if (button === 2) { // Right click to trail/play
      const pos = e.currentTarget.toLocal(e.global);
    ...
    if (isSelectingRef.current) {
      ...
      if (box) {
         ...
         const selectedIds = arrangedBlocks
            .filter(b => b.xOffset < x + w && b.xOffset + 60 > x && b.yOffset < y + h && b.yOffset + 60 > y)
            .map(b => b.id);
         useStore.setState({ selectedPocketBlockIds: selectedIds });
      }
    } else if (e.buttons === 2 || (e as unknown as { data?: { buttons?: number } }).data?.buttons === 2 || (e.nativeEvent as PointerEvent)?.buttons === 2) {
      const pos = e.currentTarget.toLocal(e.global);
  ...
  useEffect(() => {
    ...
    if (x > maxCamX) { x = maxCamX; clamped = true; }
    if (x < minCamX) { x = minCamX; clamped = true; }
    if (y > maxCamY) { y = maxCamY; clamped = true; }
    if (y < minCamY) { y = minCamY; clamped = true; }

    if (clamped) {
       useStore.getState().updatePocketCamera({ x, y });
    }
  }, [pocketCamera, containerWidth, containerHeight, gridBounds.maxX, gridBounds.maxY]);

  return (
    <div style={{ width: '100%', height: '100%', overflow: 'hidden' }} onContextMenu={e => e.preventDefault()}>
      <Application 
        backgroundAlpha={0}
