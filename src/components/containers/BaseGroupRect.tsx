  ...
  const draw = useCallback(
    (g: PIXI.Graphics) => {
      ...
      ripplesRef.current.forEach((r) => {
        ...
        g.stroke({
          width: 4,
          color: isEnabled ? 0x8b5cf6 : 0x6b7280,
      ...
      if (isSelected) {
        ...
        g.fill({
          color: isEnabled ? 0x6366f1 : 0x9ca3af,
          alpha: 0.3 * baseAlpha,
        }); // Indigo glow
        g.stroke({
          width: 3,
          color: isEnabled ? 0x4f46e5 : 0x6b7280,
          alpha: 0.8 * baseAlpha,
        });
      } else {
        // Default border
    ...
    <pixiContainer zIndex={isSelected ? 1 : 0}>
      ...
      {showGroupName && (
        <pixiText
          ...
          alpha={(isSelected ? 1 : 0.5) * (enabled !== false ? 1 : 0.3)}
          scale={0.5}
          eventMode="none"
        />
      )}
      ...
      {pitchOffset !== 0 && (
        ...
        >
          ...
          <pixiGraphics
            x={-20}
            y={-32}
            draw={(g) => drawTriangle(g, 14, "right")}
            eventMode="static"
            cursor="pointer"
            onPointerDown={(e: PIXI.FederatedPointerEvent) => {
              e.stopPropagation();
          ...
          <pixiText
            x={-40}
            y={-36}
            text={`${pitchOffset > 0 ? "+" : ""}${pitchOffset}`}
            anchor={{ x: 0.5, y: 0.2 }}
            style={{
              fontSize: 24,
              fontWeight: "bold",
