...
export const TrackRenderer: React.FC = () => {
  // Canvas state (from CanvasStoreProvider)
  const {
    tracks,
    runners,
    activeNodeDrag,
  ...
  // Store-specific state (camera, snap settings, mode)
  const { mode, activeTrackId, camera, snapToGrid } = useStore();

  const [isTrackDragging, setIsTrackDragging] = useState<{
    trackId: string;
  ...
  React.useEffect(() => {
    ...
    if (!selectedTracks.find((t) => t.id === isTrackDragging.trackId)) {
      const thisTrack = tracks.find((t) => t.id === isTrackDragging.trackId);
    ...
    const handleGlobalMove = (e: PointerEvent) => {
      ...
      const currentTrack = tracks.find(
        (st) => st.id === isTrackDragging.trackId,
      );
      if (
        currentTrack &&
        currentTrack.nodes.length > 0 &&
      ...
      const trackUpdates = selectedTracks.map((t) => {
        ...
        const newNodes = trackInitNodes.map((n) => ({
          ...n,
          x: n.x + deltaX,
          y: n.y + deltaY,
        }));
  ...
  React.useEffect(() => {
    ...
    const handleGlobalMove = (e: PointerEvent) => {
      ...
      updateTrackNode(activeNodeDrag.trackId, activeNodeDrag.nodeId, { x, y });
    };
  ...
  }, [activeNodeDrag, setActiveNodeDrag, updateTrackNode, camera, snapToGrid]);
    ...
    <>
      {visualTracks.map((track) => {
        const isSelected =
          ...
          (mode === "draw_track" && activeTrackId === track.id);
        const isActive =
          activeTrackId === track.id || mode === "draw_track" || isSelected;
        ...
        return (
          <BaseTrack
              ...
              if (e.button === 0) {
                ...
                if (track.loop === true) {
                  const dist = distToSegment(
                    x,
                    y,
                    track.nodes[track.nodes.length - 1].x,
                    track.nodes[track.nodes.length - 1].y,
                    track.nodes[0].x,
                    track.nodes[0].y,
                  );
                  if (dist < minDist) {
                    insertIdx = track.nodes.length;
                  }
                }

                const newId = insertTrackNode(track.id, insertIdx, { x, y });
                setActiveNodeDrag({
                  trackId: track.id,
                  nodeId: newId,
                  isNewNode: true,
            ...
            onNodeDoubleClick={(nodeId) => {
              const nodeIndex = track.nodes.findIndex(
                (n: TrackNode) => n.id === nodeId,
              );
              if (nodeIndex !== -1) {
                const updatedRunners = [...runners];
                const runnerIndex = updatedRunners.findIndex(
      ...
      {runners.map((runner) => {
        ...
        if (!track || track.nodes.length < 2 || track.enabled === false)
          return null;

        const isCircular = track.loop === true;
        const segmentsCount = isCircular
          ? track.nodes.length
        ...
        const x =
          aX * Math.pow(segmentT, 3) +
          bX * Math.pow(segmentT, 2) +
          cX * segmentT +
          p1.x;
        const y =
          aY * Math.pow(segmentT, 3) +
          bY * Math.pow(segmentT, 2) +
          cY * segmentT +
          p1.y;
